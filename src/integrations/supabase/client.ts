...
// Enhanced realtime activation with support for multiple tables
export const enableRealtimeForTables = () => {
  try {
    // Make sure all relevant tables are included in the realtime subscription
    const channel = supabase.channel('public-tables-changes')
      .on('postgres_changes', { 
        event: '*', 
        schema: 'public', 
        table: 'shoutouts' 
